...
namespace ARSAN_FAPI.Controllers
{
    ...
    public class PropietarioController : ControllerBase
    {
        ...
        public PropietarioController(IConfiguration config) { _config = config; }
        ...
        public IActionResult Listar()
        {
            try
            {
                ...
                using var cmd = new SqlCommand("SELECT TOP (1000) * FROM Propietario", cn);
        ...
        public IActionResult Get(int id)
        {
            try
            {
                var dt = new DataTable();
                using var cn = new SqlConnection(Conn);
                using var cmd = new SqlCommand("SELECT * FROM Propietario WHERE PropietarioID = @id", cn);
        ...
        public IActionResult Crear([FromBody] JsonElement body)
        {
            try
            {
                ...
                if (ExistsSP("sp_InsertarPropietario"))
                {
                    ExecSP("sp_InsertarPropietario", bodyDict);
                    return Ok("Insertado (SP) sp_InsertarPropietario");
                }

                var sql = "INSERT INTO Propietario(PropietarioID, PersonaID, CasaID, ClusterID) VALUES(@PropietarioID, @PersonaID, @CasaID, @ClusterID)";
                using var cn = new SqlConnection(Conn);
                using var cmd = new SqlCommand(sql, cn);

                cmd.Parameters.AddWithValue("@GuardiaID", GetSafeIntValue(bodyDict, "PropietarioID"));
                cmd.Parameters.AddWithValue("@PersonaID", GetSafeIntValue(bodyDict, "PersonaID"));
                cmd.Parameters.AddWithValue("@GuardiaID", GetSafeIntValue(bodyDict, "CasaID"));
                cmd.Parameters.AddWithValue("@PersonaID", GetSafeIntValue(bodyDict, "ClusterID"));
        ...
        public IActionResult Actualizar([FromBody] JsonElement body)
        {
            try
            {
                ...
                if (ExistsSP("sp_ActualizarPropietario"))
                {
                    ExecSP("sp_ActualizarPropietario", bodyDict);
                    return Ok("Actualizado (SP) sp_ActualizarPropietario");
                }

                var sql = "UPDATE Propietario SET PersonaID = @PersonaID WHERE PropietarioID = @PropietarioID";
                using var cn = new SqlConnection(Conn);
                using var cmd = new SqlCommand(sql, cn);
        ...
        public IActionResult Eliminar([FromQuery] int id)
        {
            ...
            {
                using var cn = new SqlConnection(Conn);
                using var cmd = new SqlCommand("DELETE FROM Propietario WHERE PropietarioID = @id", cn);
                cmd.Parameters.AddWithValue("@id", id);
                cn.Open();
                var rows = cmd.ExecuteNonQuery();
