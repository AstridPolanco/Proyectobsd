...
namespace ARSAN_FAPI.Controllers
{
    ...
    public class DetalleReciboController : ControllerBase
    {
        ...
        public DetalleReciboController(IConfiguration config) { _config = config; }
        private string Conn => _config.GetConnectionString("DefaultConnection");

        [HttpGet("Listar")] public IActionResult Listar() => Query("SELECT * FROM DetalleRecibo");
        [HttpGet("Get")] public IActionResult Get(int id) => Query($"SELECT * FROM DetalleRecibo WHERE DetalleReciboID={id}");
        [HttpPost("Crear")] public IActionResult Crear([FromBody] JsonElement body) => Insert(body, "DetalleRecibo");
        [HttpPut("Actualizar")] public IActionResult Actualizar([FromBody] JsonElement body) => Update(body, "DetalleRecibo", "DetalleReciboID");
        [HttpDelete("Eliminar")] public IActionResult Eliminar(int id) => Delete("DetalleRecibo", "DetalleReciboID", id);
